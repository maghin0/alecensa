  ...
  _initializeTable : function () {
    ...
    if ($dataColumns.length == 0) {
      ...
      this.element.find('th').each(function(index, th) {
        var thRow = $(th).parent();
        if (!thRow.children().not("TH").length && thRow.parent().prop('tagName') &&
      ...
      //If row was copied inside the THEAD remove the old one
      if (!hasThead) {
        $row.remove();
      }
    ...
    if (this.element.closest('.cmp-text').data('table-lock-first-column')) {
...
$.widget("gene.modal", {
  ...
  _initCloseActions : function() {
    this.$confirmButton.click($.proxy(function () {
      this.hide();
    }, this));

    this.$closeButtons.click($.proxy(function () {
      this.hide();
    }, this));
  },
...
(function($) {
  ...
  */
  ...
  function initHcpPopups() {
    ...
    if (!!hcpUrls) {
      ...
      if (!isHcp && $hcpModal.length) {
        $(hcpLinks).on('click', function(e) {
          ...
          $hcpModal.find('.cmp-modal__button--confirm').attr({'href': href + '#hcpm', 'target': target}).click(function() {
            sessionUtils.set(getHostName(href), true);
          });
          $hcpModal.modal('show');
        });
      }
    ...
    var currHostname = getHostName();
    var modalShown = sessionUtils.get(currHostname);
    var modalHash = window.location.hash.replace('#', '');
    ...
    if (modalHash === 'hcpm') {
      sessionUtils.set(getHostName(), true);
    ...
    if (!modalShown && isHcp && !!window.patientSiteUrl) {
      $hcpModal.find('.cmp-modal__close-icon.cmp-modal__close').attr({"href": window.patientSiteUrl, "target": target});
      $hcpModal.find('.cmp-modal__button--cancel').attr({"href": window.patientSiteUrl, "target": target});
      $hcpModal.find('.cmp-modal__button--confirm').click(function() {
        sessionUtils.set(getHostName(), true);
      });
      $hcpModal.modal('show');
    }
  ...
  function getHostName(url) {
    var hostParts = ((typeof url !== 'undefined' && url !== '' && url.indexOf('/') !== 0) ? url.replace(/(^\w+:|^)\/\//, '') : window.location.host).split("www.");
    var domainName = (hostParts.length > 1) ? hostParts[1] : hostParts[0];
...
$.widget("gene.safetyBar", {
  _create: function() {
    ...
    this.$buttonHeading = this.element.find('.cmp-safetybar__button-heading');
    this.$buttonHeadingIcon = this.element.find('.cmp-safetybar__button-heading-icon');
    ...
    if (!this.$stickyWrapper.hasClass('always-expanded')) {
      // Bind events
    ...
    if (!this.$stickyWrapper.hasClass('always-expanded')) {
      this.$buttonHeading.click($.proxy(function(event) {
        if (this.isAnimationDisabled) {
          GeneCore.utils.anchorsUtil.scrollToYPosition(this.element.offset().top, true);
        } else {
  ...
  var displayConditionalMessage = function($options) {
    ...
    if ($options.data('isMultiple')) {
      ...
      $selectedOptions.each(function() {
        var conditionalMessage = $(this).data('conditionalMessage');
