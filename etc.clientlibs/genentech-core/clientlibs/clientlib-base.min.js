  ...
  _positionHandler : function(instance, helper, position) {
    ...
    if (GeneCore.utils.resizeUtil.getCurrentBreakpoint() != 'xs') {
      ...
      if (this.element.is('IMG')) {
        ...
        var vpTop,vpBottom,eTop,eBottom;
        vpTop = helper.geo.window.scroll.top;
        vpBottom = helper.geo.window.scroll.top + helper.geo.window.size.height;
        eTop = helper.geo.origin.offset.top;
        eBottom = helper.geo.origin.offset.bottom;

        if (helper.geo.origin.windowOffset.top < 0) {
          if (eBottom < vpBottom) {
            // Case 1 - Image Top is beyond viewport top and image bottom is on viewport
            position.coord.top = (eBottom - vpTop)/2;
          } else {
            // Case 4 - Image Top is beyond viewport top and image bottom is beyond viewport bottom
            position.coord.top = (vpBottom - vpTop)/2;
          }
        } else {
          if (eBottom < vpBottom) {
            // Case 2 - Image is on viewport
            position.coord.top = (eTop - vpTop) + (eBottom - eTop)/2;
          } else {
            // Case 3 - Image Top is beyond viewport top and bottom
            position.coord.top = helper.geo.window.size.height - ((vpBottom - eTop)/2) - helper.geo.origin.size.height;
          }
        }
      } else {
            ...
            mutations.forEach(function(mutation) {
                ...
                if (nodesArray.length > 0) {
                    nodesArray.forEach(function(addedNode) {
                        if(addedNode.id !== undefined && addedNode.id.toString().indexOf('onetrust-accept-btn-handler')) {
                            $('#onetrust-accept-btn-handler, #accept-recommended-btn-handler, .save-preference-btn-handler').on("click", function(){
                                //when onetrust is click remove focus from the skip to main content button
                                $('[tabindex=0]').focus();
                            });
                        }
                    });
